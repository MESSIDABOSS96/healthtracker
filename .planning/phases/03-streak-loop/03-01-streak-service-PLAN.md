...
must_haves:
  ...
  artifacts:
    ...
    - path: "src/features/calendar/hooks.ts"
      provides: "useLiveQuery wrappers around streak.svc"
      exports: ["useMonthStreakData", "useCurrentStreakCount", "useEarliestDayKey"]
  key_links:
...
<objective>
Build the Phase 3 data foundation: a single read-only service (`streak.svc.ts`) that issues ONE `Promise.all` over four Dexie range queries to return a `Map<dayKey, QuadrantState>` for any date range; a pure date-math utility (`monthMath.ts`) that computes 42-cell month-grid windows using native `Date` routed through `src/lib/dayKey.ts`; and three `useLiveQuery` hooks that expose this data reactively to the calendar UI.

Purpose: Satisfies ARCHITECTURE.md §"Pattern for the streak calendar" — the WHOLE month grid refreshes on any write to any of the 4 tables via ONE subscription. Any per-cell query is an automatic HARD-FAIL (Anti-Pattern 3). This plan establishes the contract the entire Phase 3 UI consumes.
...
<task type="auto" tdd="false">
  <name>Task 3: Write src/features/calendar/hooks.ts — useLiveQuery wrappers</name>
  ...
  <action>
Create new file `src/features/calendar/hooks.ts`. This is a thin reactive layer — each hook is a one-liner `useLiveQuery` wrapper around `streak.svc`. Do NOT import `db` here (the `useAllFoods` exception in `src/features/food/hooks.ts:38-40` is domain-justified and documented; calendar has no equivalent need).

Required file content:

```typescript
// src/features/calendar/hooks.ts
// useLiveQuery wrappers over streak.svc. Each hook registers the 4 source tables
// (ptSessions, mealEntries, stepEntries, liftCheckins) for observation via the
// service call — so a write to ANY of them refreshes the subscribed hook. ONE
...
The `MonthStreakData` interface export is important: Plan 03-03's MonthGrid / StreakCalendar consumes it directly rather than destructuring inline, which keeps the contract explicit.

Do NOT:
- Import `db` here (not needed; all data flows through streak.svc).
- Add a `useDayDetail` hook here — that belongs to Plan 03-04 (day-detail composition), not this plan. Keep the 3-hook boundary clean.
- Memoize the `monthRangeKeys()` call with `useMemo` — it's pure and cheap; useLiveQuery's deps array `[startKey, endKey]` is what prevents re-subscription churn, not memoization of the math. (RESEARCH explicitly doesn't call for `useMemo` here.)
  </action>
  <verify>
    <automated>npx tsc --noEmit</automated>
  </verify>
  <acceptance_criteria>
    ...
    - `grep -E "export function (useMonthStreakData|useCurrentStreakCount|useEarliestDayKey)" src/features/calendar/hooks.ts` prints 3 matches
    ...
    - `! grep -E "useDayDetail" src/features/calendar/hooks.ts` (useDayDetail is owned by Plan 03-04)
    - `grep -c "useLiveQuery" src/features/calendar/hooks.ts` is 3 (one per hook, not more — no per-table splits)
    - `npx tsc --noEmit` exits 0
  </acceptance_criteria>
  <done>hooks.ts exports 3 hooks + MonthStreakData interface; each hook wraps exactly one streak.svc call; no db imports; no extra hooks; compiles.</done>
</task>

</tasks>

<threat_model>
  <scope>Read-only service layer. Four Dexie range queries + native date math. No user input reaches this plan's code.</scope>
  <inputs>
  ...
  <data_flow>UI (Plan 03-03 hooks) → useLiveQuery → streak.svc → Dexie (.between range scan on indexed dayKey). Read-only, no writes.</data_flow>
  <threats_considered>
    - S/T/R/I/D/E (STRIDE): Tampering (malformed dayKey) — not user-sourced in this plan
  ...
  <mitigations>
    ...
    - threat: "Per-cell IDB amplification (Anti-Pattern 3)"
      mitigation: "Plan delivers exactly 3 useLiveQuery subscriptions total; architectural constraint encoded in acceptance criteria `grep -c useLiveQuery` == 3"
  </mitigations>
  <residual_risk>none — pure-local read path over indexed columns; no user-influenced input in scope of this plan (Plan 03-04 owns the :dayKey route-param threat)</residual_risk>
</threat_model>
...
- Verify hook subscription count (Anti-Pattern 3 guard):
  - `grep -c useLiveQuery src/features/calendar/hooks.ts` is exactly 3
...
- Phase 3 data foundation is ready for Plan 03-03 consumption
</success_criteria>
...
- Confirmation that `useLiveQuery` count is exactly 3 and service layer has zero `useLiveQuery` calls
</output>
