...
- **A library row is only offered when it can honour the amount that was typed** (`lib/servings.ts`). A count against a weight-based row is answerable — "2 egg" is two of your usual portion — but a weight against a per-item row is not, and that candidate is dropped so the per-100g table answers instead. The old fallback silently logged "your usual" whenever the units didn't line up, so `2 egg` logged ONE egg with nothing on screen saying the 2 had gone. It got *worse* with use: before the food reached your library the same text resolved correctly off the table.
- **Both sides of a name match go through `lib/stem.ts`.** The table's search always stemmed; the library scan didn't, so `egg` hit your confirmed row and `eggs` fell through to USDA — same food, different numbers, one letter apart. That stemmer is for matching only and is deliberately not `normalizeFoodName`, whose output is the stored dedupe key.
